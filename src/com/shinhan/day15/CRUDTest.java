...
public class CRUDTest {
	...
	public static void f2(String[] args) { // 예외처리 직접함
	...
	public static void f1(String[] args) throws ClassNotFoundException, SQLException { // 예외처리 던짐
		// 1. JDBC Driver 준비(ClassPath 추가)
		// 2. JDBC Driver load
		Class.forName("oracle.jdbc.driver.OracleDriver");
		...
		while(rs.next()) {
			...
			System.out.printf("직원번호 : %d 이름 : %s 입사일 : %s comm : %3.1f\n", empid, fname, hdate, comm);
		}
		rs.close();
		st.close();
		conn.close();
